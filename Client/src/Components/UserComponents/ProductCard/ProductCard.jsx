...
const ProductCard = React.memo(({ product, AddtoCart, singleProduct, setWishList, toggleFilter }) => {
    ...
    return (
      <div className='h-fit  sm:grid sm:grid-cols-2  md:grid-cols-3 lg:grid-cols-4 lg:gap-3 lg:place-items-center xl:flex xl:gap-0 flex-wrap  xl:justify-start '>
                            {
                                product.length > 0 ? product.map((item,index) => (
                                  <div  className={` lg:card lg:block lg:h-fit  p-4 w-full lg:w-fit rounded-lg flex-none ${toggleFilter?'hidden ':''}`} key={index} >
                            
                                  <div className='relative group   xl:h-56   flex items-center justify-center lg:hover:scale-105  xl:hover:scale-110 transition duration-500' >
                                      <div className='absolute hidden lg:group-hover:flex rounded-full   bg-red-100 hover:bg-red-200 heart'>
                                  ...
                                  <div className='md: lg:w-48 xl:w-56 p-1 content bg-white'>
                                            <div className='text-xs text-slate-400 uppercase font-semibold mb-1 text-center'>{ item.category}</div>
                                            <div className=' sm:text-xs lg:text-sm uppercase font-semibold text-slate-600 text-center'>{ item.brand}</div>
                                            <div className='line-clamp-2 font-semibold  md:leading-5 lg:text-lg lg:leading-5 text-[#1e1616]  mb-2 h-fit text-center hover:text-gray-600 '><a href="/p" onClick={()=>singleProduct(item)}>{item.description }</a></div> 
                                  <div className='flex md:gap-5 xl:space-x-2 items-center sm:justify-evenly lg:justify-between'>
                                      <div className='sm:space-x-2 lg:space-x-3'>
